      ...
      <>
        {selectedPlaylist && selectedPlaylist.length > 0 ? (
          <View className="flex flex-row items-start">
              <TouchableOpacity className="pt-1 pr-2 " onPress={() => {
                setSelectPlaylist([]);
              }}>
              <X
                size={24}
          ...
          {playlists &&
            ...
            playlists.map((item) => (
              <ItemList
                key={item.id}
                title={item.name}
                onPress={() => { selectedPlaylist && selectedPlaylist.length > 0 ? handleItemLongPress(item) : handleItemPress(item) }}
